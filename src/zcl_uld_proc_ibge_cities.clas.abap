CLASS zcl_uld_proc_ibge_cities DEFINITION PUBLIC FINAL CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS: create_records IMPORTING VALUE(t_records) TYPE tt_ibge_cities
                            RETURNING VALUE(r_result)  TYPE bapiret2.

    METHODS: update_records IMPORTING VALUE(t_records) TYPE tt_ibge_cities
                            RETURNING VALUE(r_result)  TYPE bapiret2.

    METHODS: delete_records IMPORTING VALUE(t_records) TYPE tt_ibge_cities
                            RETURNING VALUE(r_result)  TYPE bapiret2.
...
ENDCLASS.
CLASS zcl_uld_proc_ibge_cities IMPLEMENTATION.
  ...
  METHOD parse_data.
    ...
    TYPES: BEGIN OF ty_raw_fields_line,
             ibge_code7      TYPE string,
             uf_code         TYPE string,
             city_name       TYPE string,
             region_name     TYPE string,
             population_2010 TYPE string,
             size_category   TYPE string,
             is_capital      TYPE string,
           END OF ty_raw_fields_line.

    DATA: ls_raw_fields_line TYPE ty_raw_fields_line.
    DATA: ls_city_line TYPE ztcidades_ibge.
    ...
    LOOP AT lt_file_lines INTO DATA(ls_file_line).
      ...
      SPLIT ls_file_line AT ';' INTO ls_raw_fields_line-ibge_code7
                               ls_raw_fields_line-uf_code
                               ls_raw_fields_line-city_name
                               ls_raw_fields_line-region_name
                               ls_raw_fields_line-population_2010
                               ls_raw_fields_line-size_category
                               ls_raw_fields_line-is_capital.

      ls_city_line-ibge_code7 = ls_raw_fields_line-ibge_code7.
      ls_city_line-uf_code = ls_raw_fields_line-uf_code.
      ls_city_line-city_name = ls_raw_fields_line-city_name.
      ls_city_line-region_name = ls_raw_fields_line-region_name.
      ls_city_line-population_2010 = ls_raw_fields_line-population_2010.
      ls_city_line-size_category = ls_raw_fields_line-size_category.
      ls_city_line-is_capital = COND #( WHEN ls_raw_fields_line-is_capital = 'Capital' THEN abap_true ELSE abap_false ).

      APPEND ls_city_line TO t_ztcidades_ibge.
  ...
  METHOD reset_table.
    ...
    SELECT * FROM ztcidades_ibge INTO TABLE @DATA(lt_ztcidades_ibge).

    DELETE ztcidades_ibge FROM TABLE @lt_ztcidades_ibge.

    FREE: lt_ztcidades_ibge.
    SELECT * FROM ztcidades_ibge INTO TABLE @lt_ztcidades_ibge.

    IF lt_ztcidades_ibge IS INITIAL.
      r_result = VALUE #( type = 'S' message = 'Tabela ZTCIDADES_IBGE Limpa!' ).
    ELSE.
      r_result = VALUE #( type = 'E' message = 'Erro Desconhecido ao tentar limpar tabela ZTCIDADES_IBGE ' ).
    ENDIF.
